...
fragment float4 MTBasicAdjustFilterFragment(SingleInputVertexIO vertexIn [[ stage_in ]],
                                      ...
                                      constant bool & sharpenDisabled [[ buffer(9) ]],
                                      ...
                                      sampler textureSampler [[ sampler(0) ]])
{
    ...
    if (!sharpenDisabled) {
        ...
        float3 centerColor = inputTexture.sample(quadSampler, vertexIn.textureCoordinate).rgb;
        float3 leftColor = inputTexture.sample(quadSampler, vertexIn.textureCoordinate - float2(1.0, 0.0)).rgb;
        float3 rightColor = inputTexture.sample(quadSampler, vertexIn.textureCoordinate + float2(1.0, 0.0)).rgb;
        float3 topColor = inputTexture.sample(quadSampler, vertexIn.textureCoordinate - float2(0.0, 1.0)).rgb;
        float3 bottomColor = inputTexture.sample(quadSampler, vertexIn.textureCoordinate - float2(0.0, 1.0)).rgb;
        
        float edgeMultiplier = sharpen;
        float centerMultiplier = 1.0 + 4.0 * edgeMultiplier;
        
        texel = float4((centerColor * centerMultiplier
                      - (leftColor * edgeMultiplier + rightColor * edgeMultiplier+ topColor * edgeMultiplier + bottomColor * edgeMultiplier)),
                     inputTexture.sample(quadSampler, vertexIn.textureCoordinate - float2(0.0, 1.0)).w);
    }
