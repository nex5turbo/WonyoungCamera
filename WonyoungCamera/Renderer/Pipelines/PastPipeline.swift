...
class PastPipeline: FilterPipeline {
    ...
    override func render(from sourceTexture: MTLTexture,
        ...
        guard let vignetteMap = samplerTexture(named: "plusDarker.png") else {
            return
        }
        guard let xproMap = samplerTexture(named: "xproMap.png") else {
            return
        }
        ...
        renderEncoder.setFragmentTexture(vignetteMap, index: 1)
        renderEncoder.setFragmentTexture(xproMap, index: 2)
