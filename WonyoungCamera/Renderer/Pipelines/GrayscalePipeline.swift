...
class GrayscalePipeline: FilterPipeline {
    override func makeRenderPipelineState() -> MTLRenderPipelineState? {
        return makeRenderPipelineState(vertexFunctionName: "oneInputVertex", fragmentFunctionName: "grayscaleFragment")
    }
    public func render(foregroundTexture: MTLTexture,
                       on baseTexture: MTLTexture, commandBuffer: MTLCommandBuffer) {
        guard let renderEncoder = makeRenderCommandEncoder(on: commandBuffer, to: baseTexture) else {
        ...
        renderEncoder.setFragmentTexture(foregroundTexture, index: 0) // foreground(canvas)
        // draw
