...
class BrightnessPipeline: FilterPipeline {
    override func makeRenderPipelineState() -> MTLRenderPipelineState? {
        return makeRenderPipelineState(vertexFunctionName: "oneInputVertex", fragmentFunctionName: "brightnessFragment")
    }
    public func render(brightness: Float, from sourceTexture: MTLTexture,
        ...
        if brightnessBuffer == nil {
            brightnessBuffer = makeBuffer(float: brightness)
        } else {
            var valueBuffer = brightness
            memcpy(brightnessBuffer?.contents(), &valueBuffer, MemoryLayout<Float>.size)
        }
        renderEncoder.setFragmentBuffer(brightnessBuffer, offset: 0, index: 0)
        // draw
