...
class ContrastPipeline: FilterPipeline {
    override func makeRenderPipelineState() -> MTLRenderPipelineState? {
        return makeRenderPipelineState(vertexFunctionName: "oneInputVertex", fragmentFunctionName: "contrastFragment")
    }
    public func render(contrast: Float,
        ...
        if contrastBuffer == nil {
            contrastBuffer = makeBuffer(float: contrast)
        } else {
            var valueBuffer = contrast
            memcpy(contrastBuffer?.contents(), &valueBuffer, MemoryLayout<Float>.size)
        }
        renderEncoder.setFragmentBuffer(contrastBuffer, offset: 0, index: 0)
        // draw
