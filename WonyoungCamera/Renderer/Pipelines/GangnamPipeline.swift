...
class GangnamPipeline: FilterPipeline {
    ...
    override func render(from sourceTexture: MTLTexture,
        ...
        guard let map = samplerTexture(named: "vintage_signature_curves1.png") else {
            return
        }
        guard let mapLgg = samplerTexture(named: "vintage_signature_lgg_curves.png") else {
            return
        }
        ...
        renderEncoder.setFragmentTexture(mapLgg, index: 2)
