...
class FrigiaPipeline: FilterPipeline {
    ...
    override func render(from sourceTexture: MTLTexture,
        ...
        guard let gradient = samplerTexture(named: "perpetua_overlay.png") else {
            return
        }
        guard let lookup = samplerTexture(named: "perpetua_map.png") else {
            return
        }
        ...
        renderEncoder.setFragmentTexture(gradient, index: 1)
        renderEncoder.setFragmentTexture(lookup, index: 2)
