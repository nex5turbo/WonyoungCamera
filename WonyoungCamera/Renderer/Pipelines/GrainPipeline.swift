...
class GrainPipeline: FilterPipeline {

    override func makeRenderPipelineState() -> MTLRenderPipelineState? {
        return makeRenderPipelineState(vertexFunctionName: "oneInputVertex", fragmentFunctionName: "grainFragment")
    }

    public func render(grain: Float, from sourceTexture: MTLTexture,
        ...
        if grainBuffer == nil {
            grainBuffer = makeBuffer(float: grain)
        } else {
            var valueBuffer = grain
            memcpy(grainBuffer?.contents(), &valueBuffer, MemoryLayout<Float>.size)
        }
        renderEncoder.setFragmentBuffer(grainBuffer, offset: 0, index: 1)
