...
class SilentPipeline: FilterPipeline {
    ...
    override func render(from sourceTexture: MTLTexture,
        ...
        guard let curves = samplerTexture(named: "sutroCurves.png") else {
            return
        }
        guard let edgeBurn = samplerTexture(named: "sutroEdgeBurn.pvr") else {
            return
        }
        guard let softLight = samplerTexture(named: "softLight.png") else {
            return
        }
        guard let sutroMetal = samplerTexture(named: "sutroMetal.pvr") else {
            return
        }
        guard let vignetteMap = samplerTexture(named: "blackOverlayMap.png") else {
            return
        }
        ...
        renderEncoder.setFragmentTexture(curves, index: 1)
        renderEncoder.setFragmentTexture(edgeBurn, index: 2)
        renderEncoder.setFragmentTexture(softLight, index: 3)
        renderEncoder.setFragmentTexture(sutroMetal, index: 4)
        renderEncoder.setFragmentTexture(vignetteMap, index: 5)
